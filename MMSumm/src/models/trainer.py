...
class Trainer(object):
    ...
    def validate(self, valid_iter, step=0):
        ...
        with torch.no_grad():
            for batch in valid_iter:
                ...
                outputs, _ = self.model(src, tgt, segs, clss, mask_src, mask_tgt, mask_cls, vgg_emb, mask_img, oscar)

                self.loss.supply_contrastive_data(oscar)
    ...
    def _gradient_accumulation(self, true_batchs, normalization, total_stats,
        ...
        for batch in true_batchs:
            ...
            # TODO
            outputs, scores = self.model(src, tgt,segs, clss, mask_src, mask_tgt, mask_cls, vgg_emb, mask_img, oscar)

            self.loss.supply_contrastive_data(oscar)
            batch_stats = self.loss.sharded_compute_loss(batch, outputs,
                    self.args.generator_shard_size, normalization)
    ...
    def test(self, test_iter, step, cal_lead=False, cal_oracle=False):
        ...
        img_path = '%s_step%d.img' % (self.args.result_path, step)
        print(img_path)
        with open(img_path, 'w') as save_imgs:
                    ...
                    with torch.no_grad():
                        for batch in test_iter:
                            ...
                            for i in range(len(img_out)):
                                print(img_out[i])
                                save_imgs.write(img_out[i].strip()+'\n')
