        ...
        if layer_cache is not None:
            if type == "self":
                ...
                if layer_cache is not None:
                    ...
                    key = shape(key)
                    value = shape(value)
            elif type == "img_context":
                query = self.linear_query(query)
                if layer_cache is not None:
                    if layer_cache["img_memory_keys"] is None:
                        key, value = self.linear_keys(key), \
                                     self.linear_values(value)
                        key = shape(key)
                        value = shape(value)
                    else:
                        key, value = layer_cache["img_memory_keys"], \
                                     layer_cache["img_memory_values"]
                    layer_cache["img_memory_keys"] = key
                    layer_cache["img_memory_values"] = value
                else:
                    key, value = self.linear_keys(key), \
                                 self.linear_values(value)
                    key = shape(key)
