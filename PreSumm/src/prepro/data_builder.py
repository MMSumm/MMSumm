...
def _format_to_bert(params):
    ...
    datasets = []
    for d in jobs:
        source, tgt, imgs, oscar = d['src'], d['trg'], d['imgs'], d['scores']
        oscar = list(map(float, oscar))
        #print(f"score : {len(oscar)} source : {len(source)}")
        ...
        if (b_data is not None):
            ...
            b_data_dict = {"src": src_subtoken_idxs, "tgt": tgt_subtoken_idxs,
                            ...
                            'src_txt': src_txt, "tgt_txt": tgt_txt, "imgs":imgs, "oscar":oscar}
