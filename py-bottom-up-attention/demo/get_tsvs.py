...
#DATA_ROOT = os.getenv('COCO_IMG_ROOT', '/scratch/summ_data/full_data/')
DATA_ROOT = os.getenv('COCO_IMG_ROOT', '/scratch/full_data_test/test_data/')
MIN_BOXES = 36
MAX_BOXES = 36

parser = argparse.ArgumentParser()
parser.add_argument('--split', default='img', help='train2014, val2014')
...
mapping = open('py-bottom-up-attention/demo/data/genome/1600-400-20/objects_vocab.txt', 'r').readlines()
m = {}
for i, e in enumerate(mapping):
    m[i] = e.split(',')[0].lower().strip()
...
def dump_features(writer, writer_labels, detector, pathXid):
    ...
    for img, img_id, instances, features in zip(imgs, img_ids, instances_list, features_list):
        try:
            ...
            full_features = np.concatenate((features.numpy(), spatial_features), axis=1)
            fea_base64 = base64.b64encode(full_features).decode('utf-8')
            fea_info = {'img_id': img_id, 'num_boxes': boxes.shape[0], 'feature': fea_base64}
...
#         }
            ...
            for i in range(len(boxes)):
                labels.append({
                    'class': m[pred_classes[i]],
                    'rect': boxes[i].tolist()
                })
                
            labels_info = {'img_id': img_id, 'list': labels}
            
            writer.writerow(fea_info)
            writer_labels.writerow(labels_info)
        except:
            continue
...
FIELDNAMES = ["img_id", "num_boxes", "feature"]
FIELDNAMES_LABELS = ["img_id", "list"]

def extract_feat(outfile, outfile_labels, detector, pathXid):
    ...
    with open(outfile, 'a') as tsvfile:
        with open(outfile_labels, 'a') as tsvfile_labels:
            writer = csv.DictWriter(tsvfile, delimiter='\t', fieldnames=FIELDNAMES)
            writer_labels = csv.DictWriter(tsvfile_labels, delimiter='\t', fieldnames=FIELDNAMES_LABELS)
            for start in tqdm.tqdm(range(0, len(pathXid), args.batchsize)):
                pathXid_trunk = pathXid[start: start + args.batchsize]
                dump_features(writer, writer_labels, detector, pathXid_trunk)
                """
                try:
...
if __name__ == "__main__":
    ...
    extract_feat('/scratch/summ_data_imgs_test/features.tsv', '/scratch/summ_data_imgs_test/labels.tsv', detector, pathXid)
